...
class EncodecModel_LFQ(nn.Module):
    ...
    def _decode_frame(self, encoded_frame: EncodedFrame) -> torch.Tensor:
        ...
        # codes = codes.transpose(0, 1)
        index = self.quantizer.decode(codes)
        emb = self.quantizer.get_codebook_entry(index)
    ...
    def _get_model(target_bandwidths: tp.List[float],
        ...
        n_q = 1 # int(1000 * target_bandwidths[-1] // (math.ceil(sample_rate / encoder.hop_length) * 10))
        quantizer = qt.LFQuantizer(
