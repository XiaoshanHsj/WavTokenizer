...
class LFQ(Module):
    def __init__(
        ...
        if self.has_projections:
            self.linear1 = torch.nn.Linear(dim, codebook_dims)
            self.linear2 = torch.nn.Linear(codebook_dims, dim)
    ...
    def forward(
        ...
        quantized = self.linear2(quantized)
        ...
        loss = 1. * commit_loss + 0.4 * entropy_aux_loss

        ret = (quantized, loss, indices)
